...
describe("Migrator", function () {
  ...
  beforeEach(async function () {
    ...
    this.farmer = await this.KukuFarmer.deploy(this.kuku.address, "1000", "0", "100000", "100000")
    await this.farmer.deployed()

    this.migrator = await this.Migrator.deploy(this.farmer.address, this.factory1.address, this.factory2.address, "0")
    await this.migrator.deployed()

    await this.kuku.transferOwnership(this.farmer.address)
  ...
  it("should do the migration successfully", async function () {
    ...
    await this.lp1.sync()
    await this.lp1.connect(this.minter).approve(this.farmer.address, "100000000000", { from: this.minter.address })
    await this.farmer.connect(this.minter).deposit("0", "2000000", { from: this.minter.address })
    expect(await this.lp1.balanceOf(this.farmer.address), "2000000")
    await expect(this.farmer.migrate(0)).to.be.revertedWith("migrate: no migrator")

    await this.farmer.setMigrator(this.migrator.address)
    await expect(this.farmer.migrate(0)).to.be.revertedWith("migrate: bad")

    await this.factory2.setMigrator(this.migrator.address)
    await this.farmer.migrate(0)
    expect(await this.lp1.balanceOf(this.farmer.address)).to.equal("0")
    expect(await this.lp2.balanceOf(this.farmer.address)).to.equal("2000000")

    await this.farmer.connect(this.minter).withdraw("0", "2000000", { from: this.minter.address })
